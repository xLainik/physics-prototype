...
local function newParticle(x, y, z, entity_dx, entity_dy, ini_angle, Particle_type)
    local self = setmetatable({}, Particle)

    self.type = Particle_type or "damage enemy"
    self.index = nil
    ...
    -- Set Particle type
    if self.type == "damage enemy" then
        self.uvs = {0/10, 0/10}
        self.speed = 100
        self.radius = 1
...
function Particle:destroyMe(external_index)
    table.insert(DELETEQUEUE, {group = "Particle", index = external_index})
