...
function camera.thirdPersonLook(dx, dy, player_x, player_y, player_z)
    ...
    camera.angle_x = camera.angle_x + dx*sensitivity
    camera.angle_z = math.max(math.min(camera.angle_z + dy*sensitivity, math.pi*0.5), math.pi*-0.5)

    if love.keyboard.isDown("t") then
        print(camera.angle_z/math.pi*180)
    end

    camera.position[1] = player_x + math.cos(camera.angle_x)*math.cos(camera.angle_z)*camera.radius
    camera.position[2] = player_y + math.sin(camera.angle_x)*math.cos(camera.angle_z)*camera.radius
    camera.position[3] = player_z + math.sin(camera.angle_z)*camera.radius

    --Equivalent code might be:
    --camera.lookAt(camera.position[1],camera.position[2],camera.position[3], player_x, player_y, player_z)

    camera.target[1] = player_x
    camera.target[2] = player_y
    camera.target[3] = player_z

    fpsController.direction = math.pi/2 - camera.angle_x
    fpsController.pitch = camera.angle_z
...
function camera.thirdPersonMovement(dt, player_x, player_y, player_z)
    ...
    if love.keyboard.isDown("left") then
        camera.angle_x = camera.angle_x - math.pi/2*dt
    elseif love.keyboard.isDown("right") then
        camera.angle_x = camera.angle_x + math.pi/2*dt
    end

    if love.keyboard.isDown("down") then
        camera.angle_z = math.max(math.min(camera.angle_z - math.pi/2*dt, math.pi*0.5), math.pi*-0.5)
    ...
    camera.target[1] = player_x + 0.3 --add the player.w/2
    camera.target[2] = player_y + 0.3 --add the player.h/2
    camera.target[3] = player_z + 0.25 --add the player.d/2
