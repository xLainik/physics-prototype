...
local function newModel(given, texture, translation, rotation, scale)
    ...
    self.matrixTransposedInverse = newMatrix()
    self.depthMVP = newMatrix()
...
--update the model's transposed and inverse matrix
function model:updateTransposedInverseMatrix()
    self.matrixTransposedInverse = transposeMatrix(invertMatrix(self.matrix, self.matrixTransposedInverse))
end

--update the model's matrix on shadow map space
function model:updateDepthMVP(camera)
    self.depthMVP = multiplyMatrices(multiplyMatrices(camera.projectionMatrix, camera.viewMatrix), self.matrix)
end
...
function model:newAnimation()
    local anim = anim9(self.anims)
    anim.animTracks = {}
    return anim
end
function model:setAnimation(anim)
    self.anim = anim
end

function model:newAnimationTrack(name)
    self.anim.animTracks[name] = self.anim:new_track(name)
end

function model:playAnimation(name)
    self.anim:play(self.anim.animTracks[name])
    self.anim:update(0)
end

function model:stopAnimation(name)
    self.anim:stop(self.anim.animTracks[name])
    self.anim:update(0)
end

function model:transitionAnimation(name, length)
    self.anim:transition(self.anim.animTracks[name],length)
    self.anim:update(0)
end

function model:resetAnimations(clear_locked)
